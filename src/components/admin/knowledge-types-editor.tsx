...
import {
  ...
  updateKnowledgeType,
  deleteKnowledgeType,
} from "@/app/(app)/admin/settings/knowledge-types/actions";

export function KnowledgeTypesEditor({
  options,
}: {
  ...
  async function handleAdd(e: React.FormEvent) {
    ...
    try {
      ...
      await createKnowledgeType(formData);
      toast.success("Knowledge type added.");
      setNewLabel("");
      router.refresh();
    } catch {
  ...
  return (
    <div className="max-w-xl space-y-3">
      {options.length === 0 ? (
        <p className="text-sm text-muted-foreground">No knowledge types yet.</p>
      ) : (
        options.map((option) => <KnowledgeTypeRow key={option.id} option={option} />)
      )}
...
function KnowledgeTypeRow({ option }: { option: LabeledOption }) {
  ...
  function handleSave() {
    startTransition(async () => {
      try {
        await updateKnowledgeType(option.id, label, sortOrder);
        toast.success("Saved.");
      } catch {
  ...
  function handleDelete() {
    ...
    startTransition(async () => {
      try {
        await deleteKnowledgeType(option.id);
